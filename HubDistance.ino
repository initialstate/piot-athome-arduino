...
// Include the DHT library for reading from the Ultrasonic Sensor
#include <NewPing.h>
...
String signalName[NUM_SIGNALS] = {":bell:Alert",":straight_ruler:Distance"};
...
// Initialize variable for determining when something is out of range
int signalError[NUM_SIGNALS];
...
void loop() 
{
  ...
  // The takeDistance() function reads from the sensor
  takeDistance();
...
void takeDistance()
{
  ...
  if (dist == 0)
  {
    signalData[0] = String("Nothing in Range");
    signalData[1] = String(dist);
    signalError[0] = 0;
    signalError[1] = 1;
  }
  else
